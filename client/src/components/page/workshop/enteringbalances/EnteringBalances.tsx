...
import scss from "./EnteringBalances.module.scss";
...
export default function EnteringBalances() {
  ...
  const listEnteringBalance = useAppSelector((state) =>
    selectorListEnteringBalance(state)
  );
...
function Option() {
  const dispatch = useAppDispatch();
  const enteringBalances = useAppSelector((state) =>
    selectorEnteringBalamces(state)
  );
  const {
    register,
    handleSubmit,
  ...
  const newRow = async (body: any) => {
    try {
      if (!enteringBalances.length)
        throw new Error("Список ввода остатков не может быть пустым");
      body.workshop = user!.workshop;
      body.userId = user!.id;
      body.type = "enteringbalances";
      body.link = 0;
      body.items = JSON.stringify(enteringBalances);
...
function List({ list }: IPropsList) {
  ...
  return (
    <div className={scss.listitems}>
      {list.map((e, i) => {
        return (
          <div className={scss.itemlist} key={i + "writedowns"}>
            <div>
              <div>
                <div>
                  <div>{e.name}</div>
            ...
            <div>
              ...
              <div onClick={() => dispatch(deleteItemForEnteringBalance(e.id))}>
                <RemoveBasketSvg
                  width="40px"
                  height="40px"
                  stroke="2"
